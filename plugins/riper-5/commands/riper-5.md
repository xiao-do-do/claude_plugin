...
**自动流转**：
- 完成所有清单项目后，自动进入审查模式（除非使用 --manual 标志）
- 在自动流转前，确认所有 todo 项目已标记为 completed
...
**默认行为**：自动流转，执行模式完成后自动进入审查和测试模式

**触发条件**：
- 执行模式：完成所有清单项目后，自动进入审查模式
- 审查模式：审查通过（实施与计划完全匹配）后，自动进入测试模式
- 测试模式：测试完成后，输出最终报告并结束流程

**流转示例**：
```
[MODE: 执行] → 完成所有清单项 → 自动进入 [MODE: 审查]
[MODE: 审查] → 实施与计划完全匹配 → 自动进入 [MODE: 测试]
[MODE: 测试] → 测试完成 → 输出报告并结束
```

**异常处理**：
- 如果审查发现偏差，停止自动流转，返回计划模式
- 如果测试发现问题，停止自动流转，返回执行模式修复
- 任何阻塞性问题都会中断自动流转，需要用户介入
