...
强制性最终步骤：将整个计划转换为编号的、顺序的清单，并使用 TaskCreate 创建任务
...
**任务创建**：
- **必须按照清单顺序依次创建任务**（从1到n+2），确保任务ID反映执行顺序
- 使用 TaskCreate 为每个清单项创建任务
- 设置清晰的 subject（简短标题）和 description（详细说明）
- 末尾必须包含"审查实施"和"测试功能"两个任务

**重要**：
1. 任务创建顺序必须与清单顺序一致，这样任务列表才能清晰反映执行流程
2. 清单末尾必须包含"审查实施"和"测试功能"两个任务，以确保完整的开发流程
...
执行前先分析规划中的任务是否有依赖关系，如无依赖使用多个 agent 并行执行提高效率

允许：
- 只实施已批准计划中明确详述的内容
- 完全按照编号清单进行
- 使用 TaskUpdate 标记任务状态（in_progress → completed）
- **团队协作**：当任务之间无依赖关系时且分布在不同文件中，创建团队并启动 team agent 并行开发提高效率

**团队协作工作流**：
1. 使用 TaskCreate 将计划清单转换为任务列表
2. 分析任务依赖关系，识别可并行执行的独立任务
3. 如有 2+ 个独立任务，使用 TeamCreate 创建团队
4. 使用 Agent 工具启动 team agent（设置 team_name 和 name 参数）
5. 使用 TaskUpdate 分配任务给 team agent（设置 owner 参数）
6. 自己处理一个任务，同时等待 team agent 完成
7. 收到 team agent 消息后验证工作，使用 SendMessage 沟通
8. 所有任务完成后，使用 SendMessage (type: "shutdown_request") 关闭 team agent
9. 使用 TeamDelete 清理团队资源
...
通过 Task API 管理开发流程：计划模式创建包含"审查实施"和"测试功能"的清单并转换为任务列表，执行模式完成后自动进入审查模式，审查通过后自动进入测试模式，测试完成后输出报告。
...
使用 Task API 跟踪进度：
- **TaskCreate**：计划模式将清单转换为任务列表
- **TaskUpdate**：执行模式标记任务为 in_progress 和 completed
- **TaskList**：查看所有任务状态和进度
